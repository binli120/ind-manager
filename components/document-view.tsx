...
export function DocumentView({ onViewChange }: DocumentViewProps) {
  ...
  const documentMetadata = currentDocument
    ? [
        { label: "Document Status", value: currentDocument.status, icon: FileText, status: "draft" },
        { label: "Document Owner", value: currentDocument.ownerName || "Unknown", icon: User },
        { label: "My Roles", value: "Author", icon: Users },
        { label: "Due Date", value: currentDocument.dueDate || "Not set", icon: Calendar },
        { label: "Last Modified", value: currentDocument.lastModified || "Unknown", icon: Clock },
      ...
      <div className="px-8 py-6">
        {isLoading ? (
          ...
          <>
            ...
            <div className="flex items-start justify-between mb-6">
              <div className="flex items-center gap-3">
                <h2 className="text-xl font-semibold text-foreground">{currentDocument.title}</h2>
                  ...
                  <div className="flex-1">
                    <div className="bg-card rounded-lg p-6 border border-border">
                      ...
                      <div className="space-y-6">
                        <div>
                          <h4 className="font-medium italic mb-4">{currentDocument.title}</h4>
                        </div>

                        <div>
                          <h5 className="font-medium mb-2">
                            {currentDocument.content || "Document content will appear here"}
                          </h5>
                        </div>
          ...
          <div className="text-center py-12">
            ...
            <p className="text-sm text-muted-foreground mt-1">Select a document from the sidebar to get started</p>
          </div>
        )}
      </div>
    </div>

...
export function SmartEditorView() {
  ...
  const [selectedSection, setSelectedSection] = useState<Section>(sectionData[0])
  const [selectedSubsection, setSelectedSubsection] = useState<SubsectionContent | null>(null)
    ...
    setSectionData((prevSections) =>
      prevSections.map((section) => {
        if (section.id === selectedSection.id) {
          ...
          const addSubsectionRecursive = (subs: SubsectionContent[]): SubsectionContent[] => {
            ...
            // If we're viewing a regular subsection's parent
              ...
              return subs.map((sub) => {
                ...
                if (sub.subsections?.some((s) => s.id === selectedSubsection.id)) {
                  shouldUpdateSelection = true
                  newParentSubsection = {
                    ...sub,
                    subsections: [...(sub.subsections || []), newSubsection],
    ...
    setSectionData((prevSections) =>
      prevSections.map((section) => {
        if (section.id === selectedSection.id) {
          ...
          const removeSubsectionRecursive = (subs: SubsectionContent[]): SubsectionContent[] => {
            return subs
              .filter((sub) => sub.id !== subsectionId)
              .map((sub) => ({
                ...sub,
                subsections: sub.subsections ? removeSubsectionRecursive(sub.subsections) : undefined,
              }))
          }
  ...
  const handleReorderSubsections = (draggedId: string, targetId: string, parentId: string | null) => {
    console.log("[v0] Reordering:", draggedId, "to", targetId, "parent:", parentId)

    setSectionData((prevSections) =>
      prevSections.map((section) => {
        if (section.id === selectedSection.id) {
          // Helper function to reorder subsections recursively
          const reorderSubsectionsRecursive = (subs: SubsectionContent[]): SubsectionContent[] => {
            ...
            return subs.map((sub) => ({
              ...sub,
              subsections: sub.subsections ? reorderSubsectionsRecursive(sub.subsections) : undefined,
            }))
          }
